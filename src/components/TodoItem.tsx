...
export default function TodoItem(props: Props) {
    return (
        <li
            className={`flex items-center justify-between p-5 mb-4 bg-white rounded-xl shadow-sm border-l-4 transition-all duration-300 hover:shadow-md ${props.todo.completed ? "border-gray-400 opacity-60" : "border-blue-500"}`}
        >
            <input
                type="checkbox"
            ...
            />
            <span
                className={`block text-lg font-bold transition-all duration-200 ${props.todo.completed ? "line-through text-gray-400" : "text-gray-800"}`}
            >
                {props.todo.text}
            </span>
            {props.todo.deadline && (
